...
import { atualizarIngresso, listarIngressosPorId } from "./ingresso.service";
import { Ingresso } from "./ingresso.types";
import { useNavigate, useParams } from "react-router-dom";
import { CurrencyInput } from "react-currency-mask";

export default function EditIngresso() {
  ...
  useEffect(() => {
    listarIngressosPorId(Number(ingressoId))
    .then((dados) => {
      setNome(dados.nome);
      setValor(dados.valor);
      setQtdLimite(dados.qtd_limite);
      setSetorId(dados.setor_id);
    });
  }, []);
  ...
  const enviarFormulario = (event: FormEvent<HTMLFormElement>) => {
    ...
    atualizarIngresso(Number(ingressoId), dados)
      .then(() => {
        form.reset();
        ...
        <div className="iq-card">
          <div className="iq-card-header d-flex justify-content-between">
            <div className="iq-header-title">
              <h4 className="card-title">Atualizar Ingresso</h4>
            </div>
          </div>
